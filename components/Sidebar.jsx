...
import { images, userDummyData } from '@/data'
  ...
  return (
    ...
    <div className='pb-5'>
        <div className='flex justify-between items-center'>
                 <img src={images.logo} alt ="logo" className='max-w-40'/>
                 <div className='relative py-2 group'>
                       <img src={images.menu_icon} alt ="logo" className='max-h-5 cursor-pointer'/>
                       <div className='absolute top-full right-0 z-20 w-32 p-5 
                                       rounded-md bg-[#282142] border border-gray-600 text-gray-100 
                                       hidden group-hover:block'>
                           <p onClick={()=> router.push("/profile")} className='cursor-pointer text-sm'>Edit Profile</p>
        ...
        <div className='bg-[#282142] rounded-full flex items-center gap-2 py-3 px-4 mt-5'>
            <img src={images.search_icon} alt="search" className='w-3'/>
    ...
    <div className='flex flex-col'>
           ...
           {userDummyData.map((user, index)=>(
                <div onClick={()=>setSelectedUser(user)} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm ${selectedUser?._id === user?._id && 'bg-[#282142]/50'}`} key={index+200}>
                    <img src={user?.profilePic || images.avatar_icon} alt="" className='w-[35px] aspect-[1/1] rounded-full'/>
